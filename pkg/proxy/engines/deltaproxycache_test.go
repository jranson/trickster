...
func TestDeltaProxyCacheRequestFastForward(t *testing.T) {
	...
	em, err := modeler.WireUnmarshaler([]byte(expectedMatrix))
	if err != nil {
		t.Error(err)
	}
	...
	ev, err := modeler.WireUnmarshaler([]byte(expectedVector))
	if err != nil {
		t.Error(err)
	}
	...
	em.SetExtents(nil)
	b, err := modeler.WireMarshaler(em)
	if err != nil {
		t.Error(err)
	}
...
func TestDeltaProxyCacheRequestCacheMissUnmarshalFailed(t *testing.T) {
	...
	modeler := client.testModeler()
	_, err = modeler.WireUnmarshaler(body)
	if err == nil {
