...
func UnmarshalTimeseriesReader(reader io.Reader,
	...
	for i := range dataStartRow - 1 {
		if len(records[i]) == 0 || !strings.HasPrefix(records[i][0], "#") {
			return nil, timeseries.ErrInvalidBody
		}
	}
	return parser.ToDataSet(records, trq)
}

// buildFieldDefinitions is the Flux CSV FieldParserFunc passed to the Parser
func buildFieldDefinitions(rows [][]string) (timeseries.SeriesFields, error) {
	l := len(rows[3])
	...
	for i := range l {
		...
		fd := loadFieldDef(rows[3][i], rows[0][i], rows[1][i], rows[2][i])
		fd.OutputPosition = i
		switch fd.Role {
...
func loadFieldDef(n, d, g, v string) timeseries.FieldDefinition {
	r := timeseries.RoleValue
	switch {
	case n == "_stop" || n == "_start" || n == "table":
		r = timeseries.RoleUntracked // is an untracked field
	case n == "time" || n == "_time":
		r = timeseries.RoleTimestamp // is a timestamp field
	case g == "true":
		r = timeseries.RoleTag // is a key/tag field
	}
...
// typeToFieldDataType is the Flux CSV DataTypeParserFunc passed to the Parser
func typeToFieldDataType(input string) timeseries.FieldDataType {
	...
	case "dateTime:RFC3339Nano", "time":
